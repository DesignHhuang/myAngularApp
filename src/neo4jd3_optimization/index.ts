          ...
          d3.select('#' + d.id + '>a#person-domain').on('click', function () {
            service.email2domain(d.properties.email).subscribe(r => {
              for (var key in r.ret_list) {
                if (r.ret_list[key]._id != d.id) {
                  var nodes = []; var relationships = [];
                  var itemnode = {
                    "id": r.ret_list[key]._id,
                    "labels": ["Domain"],
                    "properties": {
                  ...
                  var itemrelationship = {
                    ...
                    "type": "相同域名",
                    "startNode": d.id,
                    "endNode": r.ret_list[key]._id,
                    "properties": {
                      "domain": r.ret_list[key].domain ? r.ret_list[key].domain : "未知"
                    },
                    "source": d.id,
                    "target": r.ret_list[key]._id,
                    "linknum": 1
                  }
                  nodes.push(itemnode);
          ...
          d3.select('#' + d.id + '>a#person-username').on('click', function () {
            service.advanced_person_search("username:" + d.properties.username, 1, 10).subscribe(r => {
              for (var key in r.json().person) {
                if (r.json().person[key]._id != d.id) {
                  var nodes = []; var relationships = [];
                  var itemnode = {
                    "id": r.json().person[key]._id,
                    "labels": ["Person"],
                    "properties": {
                  ...
                  var itemrelationship = {
                    ...
                    "type": "相同用户名",
                    "startNode": d.id,
                    "endNode": r.json().person[key]._id,
                    "properties": {
                      "username": r.json().person[key].username
                    },
                    "source": d.id,
                    "target": r.json().person[key]._id,
                    "linknum": 1
                  }
                  nodes.push(itemnode);
                  relationships.push(itemrelationship);
          ...
          d3.select('#' + d.id + '>a#advance-ip-search').on('click', function () {
            ...
            service.getResult(searchkey, 1, 10).subscribe(r => {
              for (var key in r.json().ips) {
                if (r.json().ips[key]._id != d.id) {
                  var nodes = []; var relationships = [];
                  var itemnode = {
                    "id": r.json().ips[key]._id,
                    "labels": ["Ip"],
                    "properties": {
                  ...
                  var itemrelationship = {
                    ...
                    "type": "存在的IP",
                    "startNode": d.id,
                    "endNode": r.json().ips[key]._id,
                    "properties": {
                      "ip": r.json().ips[key].ip
                    },
                    "source": d.id,
                    "target": r.json().ips[key]._id,
                    "linknum": 1
                  }
                  nodes.push(itemnode);
                  relationships.push(itemrelationship);
